...
errtype gad_vga_init(Gadget *g, Point extent)
{
//   grs_screen *screen;
...
errtype gad_modex_init(Gadget *g, Point extent)
{
   grs_screen *screen;
...
errtype gad_svga_init(Gadget *g, Point extent)
{
   grs_screen *screen;
...
errtype gad_mono_init(Gadget *g, Point extent)
{
   Gadget *dummy;
   Point dummy2;
...
bool gad_vga_blank_expose(Region *reg, Rect *r)
{
   Rect nrect;
...
bool gad_mono_blank_expose(Region *reg, Rect *r)
{
   Region *dummy;
   Rect *dummy2;
   dummy = reg; dummy2 = r;
...
//   Spew(DSRC_UI_Utilities, ("cliprect = (%d, %d)(%d, %d)\n",a1,a2,a3,a4));
   ...
   if (f->pallOff)
   {
      ppall = (short *) (((uchar *) RefGet(REFID(id))) + f->pallOff);
...
// for that display.
Gadget *gadget_init(int display_type, Point extent)
{
   Gadget *retgad;
   GadgetData *gd;
   RectCallback fn;
...
errtype gadget_display(Gadget *g, Rect *r)
{
   return(gadget_display_part(g,r,TNG_ALLPARTS));
}

errtype gadget_display_part(Gadget *g, Rect *r, ushort partmask)
{
...
bool gadget_frob_canvas(Region *reg, void *data)
{
   Point delta;
   Gadget *g;
   ...
   gr_init_sub_canvas(g->parent->device_data, g->device_data,
...
errtype gadget_move(Gadget *g, Point coord, int z)
{
   Point delta;
...
bool gadget_tng_mono_expose(Region *reg, Rect *r)
{
   Region *dummy;
   Rect *dummy2;
   dummy = reg;
...
bool gadget_tng_vga_expose(Region *reg, Rect *r)
{
   ushort partmask;
   Rect nrect;
   int c1,c2,c3,c4;
...
errtype gadget_create_setup(Gadget **pg, Gadget *parent, GadgetClass cl, Rect *dim, int z, char *name)
{
   Gadget *retgad;
   GadgetData *gd;
   TNG *pb_tng;
