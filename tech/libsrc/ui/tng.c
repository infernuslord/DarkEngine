...
void TNGDrawBase(TNG *ptng, Point coord, Point size)
{
	short width,height;
	TNGStyle *pstyle;
	Rect rect;                        
...
void TNGDrawBitmapRef(Ref ref, Point pt)
{
	FrameDesc *pfd;
	Rect bitRect;
	Rect cvRect;
...
void TNGDrawTileMapRef(Ref ref, Point pt)
{
	FrameDesc *pfd;
	short startx;
...
bool tng_cb_mousebutt(TNG *ptng, uchar type, Point loc)
{
   TNG_CB *tngcb;
   bool retval = FALSE;
   bool newret;
...
bool tng_cb_mousemove(TNG *ptng, Point loc)
{
   TNG_CB *tngcb;
   bool retval = FALSE;
   bool newret;
...
errtype tng_cb_draw(TNG *ptng, ushort partmask, Point loc)
{
#ifndef NO_DUMMIES
   Point dummy; ushort dummy2; dummy2 = partmask; dummy = loc;
#endif
   ptng->signal(ptng, TNG_SIGNAL_EXPOSE);
   return(OK);
}

Point tng_absloc(TNG* ptng)
{
   Region* r= ((Gadget*)(ptng->ui_data))->rep;
   Point pt;
   pt.x = r->abs_x;
   pt.y = r->abs_y;
   return pt;
