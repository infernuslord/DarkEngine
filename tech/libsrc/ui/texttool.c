...
#define Cursor   _tt->lfont->cursor
...
// public useful stuff
TextTool *tt_full_build(TTRect *pos, TTState *es, TTFontInfo *ttf, void *output_data, char *keymap, void (*d_func)(void *, Rect *))
{
   TextTool *new_tt;
   char *dummy;
   dummy = keymap;
...
void _tt_new_line(long line_num)
{
   ...
   _tt->lines[line_num]=Malloc(TTL_INIT);     /* and it is empty (\0) */
...
void tt_display_all(TextTool *tt, Rect *r)
{
...
void tt_show_all(TextTool *tt)
{
   ...
   Cursor(_tt->scr_loc.crn.pt.x+_tt->cur_w,_tt->scr_loc.crn.pt.y+_tt->cur_h-_tt->disp_y);
}
...
//   mprintf ("event = %d\n  TTEV_RET = %d  TTEV_NULL = %d\n",event,TTEV_RET,TTEV_NULL);
   ...
   if (ret&TTCHG_CURSOR) Cursor(_tt->scr_loc.crn.pt.x+_tt->cur_w,_tt->scr_loc.crn.pt.y+_tt->cur_h-_tt->disp_y);
