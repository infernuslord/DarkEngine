...
// Initializes the TNG slider
errtype tng_slider_full_init(void *ui_data, TNG *ptng, TNGStyle *sty, int alignment, int min, int max, int value, int increm, Point size,
...
errtype tng_slider_2d_draw(TNG *ptng, ushort partmask, Point loc)
{
   Point size;
   Ref decid, incid;   
   Point incsize, decsize,slidsize;
   Point p1,p2,p3;
   TNG_slider *psltng;
   int xc,yc;
   float temp;
...
errtype tng_slider_size(TNG *ptng, Point *ppt)
{
   TNG_slider *psltng;
   psltng = ptng->type_data;
   ppt->x = psltng->size.x;
...
bool tng_slider_apply_click(TNG *ptng, Point loc)
{
   int perc;
   TNG_slider *psltng;
...
bool tng_slider_mousebutt(TNG *ptng, uchar type, Point loc)
{
   TNG_slider *psltng;
   bool retval = FALSE;
...
bool tng_slider_signal(TNG *ptng, ushort signal)
{
   ...
   if (signal & TNG_SIGNAL_INCREMENT)
      IF_SET_RV(tng_slider_increm(ptng->type_data));
   if (signal & TNG_SIGNAL_DECREMENT)
      IF_SET_RV(tng_slider_decrem(ptng->type_data));
