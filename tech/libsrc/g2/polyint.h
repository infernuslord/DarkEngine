/*
 * $Source: x:/prj/tech/libsrc/g2/RCS/polyint.h $
 * $Revision: 1.2 $
 * $Author: TOML $
 * $Date: 1996/10/11 12:06:44 $
 *
...
do {                                                          \
   ...
   du = fix_div(fix_mul((*p_next)->u,w0+fix_mul(dw,_d)) - u0, _d); \
   u0 += fix_mul(du,fix_ceil(_prev->y)-_prev->y);              \
   v0 = fix_mul(_prev->v,w0);                                 \
   dv = fix_div(fix_mul((*p_next)->v,w0+fix_mul(dw,_d)) - v0, _d); \
   v0 += fix_mul(dv,fix_ceil(_prev->y)-_prev->y);              \
   w0 += fix_mul(dw,fix_ceil(_prev->y)-_prev->y);              \
} while (0)
