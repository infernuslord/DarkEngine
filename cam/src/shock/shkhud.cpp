// $Header: r:/t2repos/thief2/src/shock/shkhud.cpp,v 1.17 2000/02/19 13:25:27 toml Exp $

// Shock HUD overlay elements
...
void ShockHUDInit(int which)
{
   ...
   // ShockOverlaySetRect(which,interface_rect);
   sScrnMode smode;
   ScrnModeGet(&smode);
   ...
   for (cHUDRectList::cIter iter = hud_rects.Iter(); !iter.Done(); iter.Next())
   {
      ...
      if (o != OBJ_NULL)
      {
         ...
         if (ObjGetHitPoints(o,&hp) && ObjGetMaxHitPoints(o,&hpmax) && drawhp)
         {
            ...
            gr_font_string(gShockFont, temp, x, y);
            */
            // draw a goofy meter
         ...
         if (IsAPlayer(o))
         {
            ...
            if (strlen(pName) > 0)
            {
               ...
               gr_font_string(gShockFontMono, (char *) pName, 
                              r.ul.x + 6, r.ul.y + 3);
            }
         }
      }
...
void ShockHUDDrawRect(Rect r, ObjID o)
{
   sHUDElem hudelem;
   //int dx,dy;
   Rect use_rect;
