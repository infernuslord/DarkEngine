// $Header: r:/t2repos/thief2/src/shock/shksecur.cpp,v 1.10 2000/02/19 13:26:14 toml Exp $
...
//--------------------------------------------------------------------------------------
void ShockSecurityInit(int which)
{
   int i;
...
void ShockSecurityDraw(void)
{
   ...
   r= ShockOverlayGetRect(kOverlaySecurity);
   player = PlayerObject();
   ...
   keyobj = pShockPlayer->GetEquip(player, kEquipFakeKeys);
   if (keyobj == NULL)
      return;
   ...
   for (i=0; i < 32; i++)
   {
      ...
      if (access & bit)
      {
         ShockStringFetch(temp,sizeof(temp),"access","misc",bitshift[i]);
...
//--------------------------------------------------------------------------------------
bool ShockSecurityHandleMouse(Point mpt)
{
   return(TRUE);
}
//--------------------------------------------------------------------------------------
...
{
   Rect r= ShockOverlayGetRect(kOverlaySecurity);
...
//--------------------------------------------------------------------------------------
void ShockSecurityStateChange(int which)
{
...
//--------------------------------------------------------------------------------------
bool ShockSecurityCheckTransp(Point pos)
{
   return(FALSE);
}
//--------------------------------------------------------------------------------------
