// $Header: r:/t2repos/thief2/src/shock/shkkeypd.cpp,v 1.33 2000/02/19 13:25:34 toml Exp $
...
//--------------------------------------------------------------------------------------
void ShockKeypadClear(void)
{
   keypad_num = 0;
   keypad_digits = 0;
}
//--------------------------------------------------------------------------------------
void ShockKeypadInit(int which)
{
   //char temp[40];
   int i; // ,s;
   ...
   */
   ...
   gKeypadFont->pal_id = gShockFontAA->pal_id;
...
//--------------------------------------------------------------------------------------
void ShockKeypadDrawCore(void)
{
   int dx,dy; // ,i;
   //int w,h;
   char format[32], temp[255];
   //Rect r2 = ShockOverlayGetRect(kOverlayKeypad);
   Rect r = ShockOverlayGetRect(kOverlayKeypad);
   /*
   Rect r;
...
void KeypadButton(int button_num)
{
   if (button_num >= 0)
   {
      ...
      if (keypad_digits == 5)
      {
         uiDefer(DeferKeypad, (void *)ShockOverlayGetObj());
      }
   }
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
bool ShockKeypadHandleMouse(Point pt)
{
   return(TRUE);
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
void ShockKeypadStateChange(int which)
{
...
//--------------------------------------------------------------------------------------
void ShockKeypadOpen(ObjID o)
{
   ShockOverlayChangeObj(kOverlayKeypad, kOverlayModeOn,o);
   ShockKeypadClear();
}
//--------------------------------------------------------------------------------------
// Check for transparency in the source art
//--------------------------------------------------------------------------------------
bool ShockKeypadCheckTransp(Point pt)
{
   return(FALSE); 
   //return(ShockOverlayCheckTransp(pt,kOverlayKeypad, gKeypadBack));
}

sOverlayFunc OverlayKeypad = { 
   ShockKeypadDraw, // draw
   ShockKeypadInit, // init
   ShockKeypadTerm, // term
   ShockKeypadHandleMouse, // mouse
   ...
   "", // downschema
   ShockKeypadStateChange, // state
   ShockKeypadCheckTransp, // transparency
   TRUE, // distance
   TRUE, // needmouse
};
