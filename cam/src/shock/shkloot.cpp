// $Header: r:/t2repos/thief2/src/shock/shkloot.cpp,v 1.9 1999/08/05 17:48:38 Justin Exp $
...
{
   int total, randval, count, choose;
   int i;
   ...
   count = 0;
   for (choose = 0; choose < MAX_LOOT_ITEMS; choose++)
   {
...
void GenerateLoot(ObjID corpse)
{
   ...
   statval = 1; // disabled "luck" treasure system
   for (n=0; n < statval; n++)
   {
      for (i=0; i < MAX_LOOT_ITEMS; i++)
      {  
      ...
      AutoAppIPtr(ShockPlayer);
      if (gPropGuaranteedLoot->IsRelevant(corpse) && pShockPlayer->HasTraitMulti(kTraitBorg))
      {
         Label *pLabel;
...
// Damage Listener
//
eDamageResult LGAPI ShockLootDamageListener(const sDamageMsg* pMsg, tDamageCallbackData)
{
   switch(pMsg->kind)
   {
