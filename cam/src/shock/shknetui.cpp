// $Header: r:/t2repos/thief2/src/shock/shknetui.cpp,v 1.7 2000/02/19 13:25:48 toml Exp $
#include <shknetui.h>
#include <drkpanl.h>
#include <metasnd.h>
...
public: 
   ...
   cMultiplayUIPage(const sDesc& desc, cShockMultiplayUI* panel); 
...
protected: 
   ...
   // Data members
   //
   cShockMultiplayUI* mpPanel; // the UI panel
...
cMultiplayUIPage::cMultiplayUIPage(const sDesc& desc, cShockMultiplayUI* panel)
...
int cMultiplayUIPage::DrawString(const char* str, const Rect& relr, ulong flags, guiStyle* style, StyleFontKind font)
{
   ...
   return ShockDrawString(str,r,flags,style,font); 
}
...
public:
   cMPFrontPage(cShockMultiplayUI* pUI)
      : cParent(gDesc,pUI)
   {
   ...
   static cMultiplayUIPage* Create(cShockMultiplayUI* pUI)
   {
...
public:
   cMPGameNamePage(cShockMultiplayUI* pUI)
     : cParent(gDesc,pUI),
   ...
   BOOL ExchangeData(eDataExchange op)
   {
      ...
      switch (op)
      {
         ...
         case kSaveAndValidate:
            ...
            ShockSetHostName(mGame.GetText()); 
...
public:
   cMPHostAddressPage(cShockMultiplayUI* pUI)
     : cParent(gDesc,pUI),
   ...
   void OnNetMsg(eNetListenMsgs msg, DWORD data)
   {
      ...
      switch (msg)
      {
         case kNetMsgHi:
            ...
            {
               mJoinState = kJoined; 
               mpPanel->SpoofUserClick(cShockMultiplayUI::kNext); 
            }
            break; 
...
public:
   cMPLobbyBase(const sDesc& desc, cShockMultiplayUI* pUI)
     : cParent(desc,pUI),
       mNumPlayers(0)
   {
...
public:
   cMPHostLobbyPage(cShockMultiplayUI* pUI)
      : cParent(gDesc,pUI)
   {
...
public:
   cMPClientLobbyPage(cShockMultiplayUI* pUI)
      : cParent(gDesc,pUI)
   {
...
public:
   cMPStartPage(BOOL host, cShockMultiplayUI* pUI)
   ...
   static cMultiplayUIPage* CreateHost(cShockMultiplayUI* pUI)
   {
      return new cMPStartPage(TRUE,pUI); 
   }

   static cMultiplayUIPage* CreateClient(cShockMultiplayUI* pUI)
   {
...
typedef cMultiplayUIPage* (*tPageFactory)(cShockMultiplayUI* ); 
...
cShockMultiplayUI::cShockMultiplayUI()
   : cParent(&gDesc),
     mPageIdx(0),
...
void cShockMultiplayUI::RedrawDisplay()
{
...
void cShockMultiplayUI::OnButtonList(ushort action, int button)
{
   if (!(action & BUTTONGADG_LCLICK))
      return ;
...
void cShockMultiplayUI::SpoofUserClick(int button)
{
...
void cShockMultiplayUI::ChangePage()
{
   if (mPageDelta == 0)
      return; 
...
void cShockMultiplayUI::OnLoopMsg(eLoopMessage msg, tLoopMessageData data)
{
...
void cShockMultiplayUI::SetupPage(BOOL draw)
{
   if (!mpPage)
      return ;
   mpPage->InitUI(); 
...
void cShockMultiplayUI::CleanupPage()
{
   if (mpPage)
      mpPage->TermUI(); 
...
void cShockMultiplayUI::SetupButtons()
{
...
cMultiplayUIPage* cShockMultiplayUI::CreatePage(int page_id)
{
...
void cShockMultiplayUI::SetPages(int* pages, int idx)
{
   mpPageList = pages;
   mPageIdx = idx; 
}
...
void cShockMultiplayUI::OnEscapeKey()
{
   if (mpPage)
      mpPage->ExchangeData(kAbort);
   gpNetMan->Leave(); 
...
void cShockMultiplayUI::net_cb(eNetListenMsgs msg, DWORD dat, void* data)
{
   cShockMultiplayUI* us = (cShockMultiplayUI*)data;
   us->OnNetMsg(msg,dat); 
}

void cShockMultiplayUI::OnNetMsg(eNetListenMsgs msg, DWORD data)
{
...
//------------------------------------------------------------
// Global cShockMultiplayUI stuff
//
...
sDarkPanelDesc cShockMultiplayUI::gDesc = 
{
   "netplay",
   cShockMultiplayUI::kNumButts,
   cShockMultiplayUI::kNumRects,
   cShockMultiplayUI::kNumButts,
   button_names, 
   NULL, // font
   0,
...
static cShockMultiplayUI* gpMultiplayUI = NULL; 

void SwitchToShockMultiplayUIMode(BOOL push) 
{
   if (gpMultiplayUI)
   {
...
void ShockMultiplayUIInit()
{
   gpMultiplayUI = new cShockMultiplayUI; 
   gpStartGameMsg = new cNetMsg(&sStartGameDesc);
}

void ShockMultiplayUITerm()
{
