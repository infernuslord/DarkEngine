// $Header: r:/t2repos/thief2/src/shock/shkemail.cpp,v 1.26 2000/02/19 13:20:18 toml Exp $
...
//--------------------------------------------------------------------------------------
void ShockEmailDraw(void)
{
   Rect r = ShockOverlayGetRect(kOverlayEmail);
   Point drawpt;
   int dx,dy,h;
   grs_clip saveclip;
   ...
   gr_font_string(gShockFont,gEmailHeader,dx,dy);
   h = gr_font_string_height(gShockFont,gEmailHeader);

   gr_font_string(gShockFont,gEmailText,dx,dy+h+EMAIL_SEPARATOR);
...
//--------------------------------------------------------------------------------------
bool ShockEmailHandleMouse(Point pt)
{
   return(FALSE);
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
void ShockEmailPlayAudio(char *artname)
{
...
//--------------------------------------------------------------------------------------
void ShockEmailDisplay(int mode, int level, int which, BOOL show_mfd)
{
   char artname[256];
   char findname[64];
   char levelname[64];
   ...
   if (show_mfd)
   {
      ...
      ShockStringFetch(artname,sizeof(artname),findname,levelname,which+1);
      ...
      ShockStringFetch(gEmailText,sizeof(gEmailText),findname,levelname,which+1);
      gr_font_string_wrap(gShockFont,gEmailText,TEXT_W);

      strcpy(gEmailHeader,"");
      sprintf(findname,"%sName",mode_roots[gEmailMode]);
      ShockStringFetch(gEmailHeader,sizeof(gEmailHeader),findname,levelname,which+1);
      gr_font_string_wrap(gShockFont,gEmailHeader,TEXT_W);

      ShockOverlayChange(kOverlayEmail, kOverlayModeOn);
   }
...
static bool email_cb(ushort action, int button, void* data, LGadBox* vb)
{
   ...
   case kEmailButtonPageDown:
      ...
      h = gr_font_string_height(gShockFont, gEmailText) + gr_font_string_height(gShockFont, gEmailHeader) + EMAIL_SEPARATOR;
      if (h > TEXT_H)
      {
...
//--------------------------------------------------------------------------------------
void ShockEmailStateChange(int which)
{
...
//--------------------------------------------------------------------------------------
bool ShockEmailCheckTransp(Point pt)
{
   return(FALSE);
}
//--------------------------------------------------------------------------------------
// needs a better return type!
int ShockFindLogData(ObjID obj, int usetype, int *level, int *which)
{
...
sOverlayFunc OverlayEmail = { 
   ...
   "", // upschema
   "", // downschema
   ShockEmailStateChange, // state
   ShockEmailCheckTransp, // transparency
   0, // distance
   TRUE, // needmouse
};
