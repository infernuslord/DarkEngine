// $Header: r:/t2repos/thief2/src/engfeat/gunprop.cpp,v 1.8 1999/12/10 14:28:03 BFarquha Exp $
...
#define BASE_GUN_FIELD(i) \
   ...
   {"Setting " #i ": Zoom Factor", kFieldTypeFloat, FieldLocation(cBaseGunDescs, m_zoom[i]),}

// Must keep this is synch with kNumGunSettings
static sFieldDesc BaseGunDescFields[] =
{
   {"Number of Settings", kFieldTypeInt, FieldLocation(cBaseGunDescs, m_numGunSettings),},
   BASE_GUN_FIELD(0),
   BASE_GUN_FIELD(1),
   BASE_GUN_FIELD(2),
};
...
float BaseGunDescGetZoom(ObjID objID)
{
   cBaseGunDescs *pBaseGunDesc;

   if (g_baseGunDescProperty->Get(objID, &pBaseGunDesc))
   {
      float val = pBaseGunDesc->m_zoom[GunStateGetSetting(objID)];
      if (val) return val;
   }
   return 1.0;
}

int BaseGunDescGetNumSettings(ObjID objID)
{
   cBaseGunDescs *pBaseGunDesc;

   if (g_baseGunDescProperty->Get(objID, &pBaseGunDesc))
   {
      return pBaseGunDesc->m_numGunSettings;
   }
   return 0;
}
...
float AIGunDescGetMaxRange(ObjID objID)
{
   ...
   return kShockGunDefaultMaxRange;
}
...
BOOL GunKickGet(ObjID objID, sGunKick **ppGunKick)
{
   ...
   {
      *ppGunKick = &(pGunKicks->m_kick[GunStateGetSetting(objID)]);
      return TRUE;
   }
   return FALSE;
}
...
static sFieldDesc GunReliabilityFields[] =
{
   ...
   {"Silencing Degrade Rate (%)", kFieldTypeFloat, FieldLocation(sGunReliability, m_silenceDegradeRate), kFieldFlagNone,},
...
static sFieldDesc GunStateFields[] =
{
   ...
   {"Silencing (%)", kFieldTypeFloat, FieldLocation(sGunState, m_silenceValue),},
...
static sPropertyTypeDesc weaponTypeTypeDesc = {PROP_SHOCK_WEAPON_TYPE_NAME, sizeof(int)};
