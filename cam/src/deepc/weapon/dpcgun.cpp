...
ObjID GunUnload(ObjID gunID, uint flags)
{
    ...
    if (clipID != OBJ_NULL)
    {
        ...
        if (flags & LF_AUTOPLACE)
            DPCInvAddObj(PlayerObject(),unloadID);
        else if (flags == LF_NONE)
            DPCInvLoadCursor(unloadID);
...
void GunLoad(ObjID gunID, ObjID clipID, uint flags)
{
    ...
    if (currentAmmoCount+ammoCount>clip)
    {
        ...
        {
            DPCInvAddObj(PlayerObject(),clipID);      
        }
    }
    else
    {
    ...
    {
        DPCInvAddObj(PlayerObject(),unload_obj);         
    }
}
...
float GunGetSilencing(ObjID gunID)
{
//guncondition code changed, copying the GunGetCondition from 
//deepc\weapon\dpcgun.cpp AMSD
   sGunState *pState;
...
void GunGetConditionString(ObjID gunID, char *buf, int buflen)
{
   ...
   DPCStringFetch(buf, buflen, "GunCondVal", "weapon", val + 1);
}

////////////////////////////////////////////////////////////////

#define MAX_GUNSIL_STRING 10
#define GUNSIL_INTERVAL   10
void GunGetSilenceString(ObjID gunID, char *buf, int buflen)
{
   float cond = GunGetSilencing(gunID);
   int val;

   val = ((int)cond)/GUNSIL_INTERVAL;
   if (val < 0)
      val = 0;
   if (val > MAX_GUNSIL_STRING - 1)
      val = MAX_GUNSIL_STRING -1;

   DPCStringFetch(buf, buflen, "GunSilenceVal", "weapon", val + 1);
}
...
void GunSetSilencing(ObjID gunID, float silenceVal)
{
   sGunState *pGunState;

   if (g_pGunStateProperty->Get(gunID, &pGunState))
     {
       pGunState->m_silenceValue = silenceVal;
       g_pGunStateProperty->Set(gunID, pGunState);
     }
   else
      Warning(("GunSetCondition: Gun %d has no gun state\n"));
