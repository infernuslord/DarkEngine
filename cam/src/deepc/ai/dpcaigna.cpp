// Deep Cover AI Action - gun attack
//
...
{
   AutoAppIPtr_(NetManager, pNetMan);
   sAINetMsg_DPCShoot *netmsg = (sAINetMsg_DPCShoot *)pMsg;
...
void cAIGunAction::Shoot(void)
{
   ...
   ObjID projID = GetProjectile(m_gunID);
   ...
   // since we're already networking down in DPCLaunchProjectile. Once
   ...
   if (m_gunJointID != -1)
   {
      ...
      DPCLaunchProjectile(m_pAIState->GetID(), projID, &launchParams);
...
STDMETHODIMP_(eAIResult) cAIGunAction::Enact(ulong deltaTime)
{
    ...
    // since the lower level: DPCLaunchProjectile() will be broadcast.
