...
class cDPCCamera: 
public cCTDelegating<IDPCCamera>, 
...
public:
    ...
    m_cameraSpeed(kDefaultCameraSpeed)
    {
        ...
        MI_INIT_AGGREGATION_1(pOuter, IDPCCamera, kPriorityNormal, NULL);
    }
...
STDMETHODIMP cDPCCamera::SetLocation(const mxs_vector *pPos, const mxs_angvec *pAng)
{
...
STDMETHODIMP_(BOOL) cDPCCamera::Attach(ObjID objID)
{
    CameraAttach(PlayerCamera(), objID);
    return TRUE;
}

STDMETHODIMP_(BOOL) cDPCCamera::Attach(const Label *pName)
{
    ObjID objID;
...
STDMETHODIMP_(BOOL) cDPCCamera::Attach(const char *pName)
{
    return Attach((const Label*)pName);
}

STDMETHODIMP cDPCCamera::Detach(void)
{
    CameraDetach(PlayerCamera());
    return S_OK;
}

STDMETHODIMP_(ObjID) cDPCCamera::GetAttachObject(void)
{
    return CameraGetObjID(PlayerCamera());
}

STDMETHODIMP cDPCCamera::SetScene(const char *pSceneName)
{
    m_sceneName.Empty();
...
{
    DPCLetterboxSet(camera);
}

STDMETHODIMP cDPCCamera::SetEditMode(eCameraEditType mode)
{
...
STDMETHODIMP cDPCCamera::SetEditState(eCameraEditType state)
{
    m_camState = state;
    return S_OK;
}

STDMETHODIMP_(eCameraEditType) cDPCCamera::GetEditMode(void)
{
    return m_camMode;
}

STDMETHODIMP_(eCameraEditType) cDPCCamera::GetEditState(void)
{
    return m_camState;
}

STDMETHODIMP cDPCCamera::Frame(void)
{
    switch (m_camMode)
    {
        case kCameraRecord:
...
void cDPCCamera::ScanScene(const char *pszDataPath, const char *pszWhat)
{
    Datapath sDatapath;
...
void cDPCCamera::AddSample(int cameraNum, const sPositionSample *pSample)
{
...
HRESULT cDPCCamera::LoadSamples(const char *pFileName)
{
...
void cDPCCamera::DestroyAllSamples(void)
{
...
void cDPCCamera::LoadAllSamples(void)
{
    cStr fullName;
    cFileName *pCameraFile;
...
HRESULT cDPCCamera::LoadScene(const char *pszDataPath, const char *pszWhat)
{
    Datapath sDatapath;
    DatapathDir *pDatapathDir;
    char *pszFileName;
...
HRESULT cDPCCamera::SaveScene(const char *pName)
{
    FILE *pFile;
    sSceneHeader header;
    cStr fileName;
...
HRESULT cDPCCamera::LoadScene(const char *pName)
{
...
void cDPCCamera::ScanScene(const char *pName)
{
    static char szDataPath[PATH_MAX];
    cStr resCameraPath;
...
HRESULT cDPCCamera::SaveSamples(const char *pszWhat)
{
    cStr fileName;
    tSimTime t;
    int currentSample;
...
HRESULT cDPCCamera::FindNextFreeCameraFile(void)
{
    BOOL found;
    int i;
...
void cDPCCamera::RestoreDefaultCamera(void)
{
...
void cDPCCamera::StartSampling(void)
{
    m_numRawSamples = 0;
}

void cDPCCamera::SwitchToNewCamera(void)
{
    AutoAppIPtr(ObjectSystem);
    cStr camName;
...
void cDPCCamera::DumpSceneInfo(void)
{
...
STDMETHODIMP cDPCCamera::SetCameraSpeed(float speed)
{
    m_cameraSpeed = speed;
    return S_OK;
}

STDMETHODIMP_(float) cDPCCamera::GetCameraSpeed(void)
{
    return m_cameraSpeed;
}

void cDPCCamera::UpdateCameraPos(void)
{
...
void cDPCCamera::MoveCameraToPlayer(void)
{
...
void cDPCCamera::PlaybackSwitch(void)
{
    if (m_numSwitches>0)
    {
...
HRESULT cDPCCamera::SaveSwitches(const char *pszWhat)
{
    cStr fileName;
...
HRESULT cDPCCamera::LoadSwitches(const char *pszWhat)
{
    cStr fileName;
    tSimTime time;
    int cameraNum;
...
void cDPCCamera::RecordSwitch(tSimTime time, int cameraNum)
{
...
STDMETHODIMP cDPCCamera::SwitchCamera(int cameraNum)
{
...
STDMETHODIMP_(int) cDPCCamera::GetNumCameras(void)
{
    return m_numLoadedCameras;
}

void cDPCCamera::ScanScenes(cFileNameList &sceneList, const char *pDataPath)
{
    Datapath sDatapath;
    DatapathDir *pDatapathDir;
    cStr wildCardStr;
...
STDMETHODIMP cDPCCamera::MakeScenes(void)
{
...
{
    AutoAppIPtr(Unknown); 
    new cDPCCamera(pUnknown); 
}

static float zoomTarget = 1.0;
static float zoomRate; // In zoom-multiply-per-second
...
void ZoomIntegrate(void) {
   ...
   float diff = zoomTarget - cam->zoom;
   if (diff) {
      if (!zoomRate) {
         // immediate zoom
         cam->zoom = zoomTarget;
      } else {
         float multFact = pow(zoomRate, (GetSimFrameTime()/1000.0));
         if (diff > 0) {
            cam->zoom *= multFact;
