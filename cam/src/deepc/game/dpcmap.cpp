...
static IRes *gMapHnd         = NULL;
static IRes *gMiniMapHnd     = NULL;
static IRes *gAutoMapHnd     = NULL;
static IRes *gMiniAutoMapHnd = NULL;
static IRes *gMapNone        = NULL;
static IRes *gMapBack        = NULL;
static IRes *gOnHnd          = NULL;
static IRes *gOffHnd         = NULL;
static IRes *gBrackets       = NULL;
...
static void DrawObjectInMapCoords(Point dp, ObjID o, IRes *hnd, BOOL rotate, BOOL automap)
{
    ...
    if (DPCOverlayCheck(kOverlayMap))
    {
        if (gSelectedObj == o)
        ...
        Point mpos;
        int dx,dy;
        Rect r = DPCOverlayGetRect(kOverlayMap);
    ...
    //mprintf("minimap check is %d\n",DPCOverlayCheck(kOverlayMiniMap));
    if (gPropMapText->IsRelevant(o) && DPCOverlayCheck(kOverlayMap))
    {
        const char *pStr;
        char temp[255];
        int w,h;
        ...
        gr_font_string(gDPCFont,temp,drawpt.x - (w / 2), drawpt.y + 10);
...
// buffer -- X
void DPCMapDrawContents(Point dp, IRes *back, BOOL automap)
{
    int maps = 0;
    char buf[64]; 
    char temp[64];
    ...
    else
    {
        ...
        char lvl[255];
        DPCGetLevelName(lvl,sizeof(lvl));
...
void DPCMapDrawCore(void)
{
    ...
    automap = pDPCPlayer->HasTrait(PlayerObject(),kTraitAutomap);
    if (automap)
        DPCMapDrawContents(drawpt, gAutoMapHnd, automap);
    else
        DPCMapDrawContents(drawpt, gMapHnd, automap);
    ...
    if (gExplored.explored[maploc] || automap)
    {
        ...
        DPCStringFetch(text,sizeof(text),tagname,"maptext",maploc);
        if (strlen(text) > 0)
        {
            short w;
            w = gr_font_string_height(gDPCFont,text);
            gr_font_string(gDPCFont,text,MAPTEXT_X + ((MAPTEXT_W - w) / 2), MAPTEXT_Y);
        }
    }
}
...
void DPCMapDraw(unsigned long inDeltaTicks)
{
    Rect r = DPCOverlayGetRect(kOverlayMap);
    ...
    if (DPCOverlayCheck(kOverlayMiniMap))
...
//--------------------------------------------------------------------------------------
bool DPCMapHandleMouse(Point pos) 
{
...
//--------------------------------------------------------------------------------------
void DPCMapStateChange(int which)
{
    gSelectedObj = OBJ_NULL;
...
//--------------------------------------------------------------------------------------
void DPCMapSetExplored(int loc, char val)
{
...
//--------------------------------------------------------------------------------------
void DPCMiniMapTerm(void)
{
    SafeFreeHnd(&gMinimapHnd);
}
//--------------------------------------------------------------------------------------
static int gMapLoc = -1;
void DPCMiniMapDraw(unsigned long inDeltaTicks)
{
    ...
    if (maploc >= 0 && maploc < tmpRects.Size())
    {
        ...
        */
        ...
        if (drawpt.y < r2.lr.y - bh)
            drawpt.y = r2.lr.y - bh;
        if (pDPCPlayer->HasTrait(PlayerObject(),kTraitAutomap))
            DPCMapDrawContents(drawpt, gMiniAutoMapHnd, TRUE);
        else
            DPCMapDrawContents(drawpt, gMiniMapHnd, FALSE);
...
//--------------------------------------------------------------------------------------
void DPCMiniMapStateChange(int which)
{
    gMapLoc = -1;
    if (DPCOverlayCheck(which))
    {
...
//--------------------------------------------------------------------------------------
bool DPCMiniMapCheckTransp(Point pt)
{
...
//--------------------------------------------------------------------------------------
IRes *DPCMiniMapBitmap(void)
{
    return(gMinimapHnd);
}
//--------------------------------------------------------------------------------------
bool DPCMiniMapUpdate(void)
{
...
{ 
    ...
    NULL,                     // dragdrop
    NULL,                     // key
    DPCMiniMapBitmap,         // bitMiniMap
    "",                       // upschema
    "",                       // downschema
    DPCMiniMapStateChange,    // state
    NULL,                     // transparency
    0,                        // distance
    FALSE,                    // needmouse
    200,                      // alpha
    DPCMiniMapUpdate,         // update
};
