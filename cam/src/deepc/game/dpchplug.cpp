...
void DPCHRMPlugInit(int which)
{
   ...
   if (smode.w > 640)
   {
      ...
      DPCOverlaySetRect(kOverlayHRMPlug, use_rect);
   }
   else
      DPCOverlaySetRect(kOverlayHRMPlug, full_rect);

}

//--------------------------------------------------------------------------------------
void DPCHRMPlugTerm(void)
{
   int s,i;
...
//--------------------------------------------------------------------------------------
void DPCHRMPlugDraw(unsigned long inDeltaTicks)
{
   Rect r = DPCOverlayGetRect(kOverlayHRMPlug);
...
{
   DPCHRMDisplay(gPlugObj, gPlugMode, FALSE, gPlugBonus);
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
void DPCHRMPlugStateChange(int which)
{
...
//--------------------------------------------------------------------------------------
void DPCHRMPlugSetMode(int mode, ObjID obj, int bonus)
{
   gPlugMode = mode;
   gPlugObj = obj;
   gPlugBonus = bonus;
...
//--------------------------------------------------------------------------------------
bool DPCHRMPlugCheckTransp(Point pt)
{
   return(DPCOverlayCheckTransp(pt,kOverlayHRMPlug, gPlugBacks[gPlugMode]));
}
