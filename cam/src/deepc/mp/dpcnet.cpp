...
// These are all for DPCPlayerCrash:
#include <iobjnet.h>
#include <contain.h>
#include <rendprop.h>
#include <physapi.h>
...
void DPCSetHostName(const char *name)
{
...
void DPCSendAddText(ObjID player, const char *pText, int time)
{
   if (player == OBJ_NULL) {
...
{
   DPCContainerCheckRemove(o, cont);
...
// Host code
//
void DPCBroadcastRemoveContainee(ObjID o, ObjID cont)
{
   if (cont == OBJ_NULL)
      return;
...
static void handleHilightObject(ObjID obj, ObjID from)
{
   if (!IsPlayerObj(from))
   {
      ...
      char temp[255];
      if (DPCStringFetch(temp, sizeof(temp), "HilightString", "misc"))
      {
         char buf[255];
         sprintf(buf, temp, objName, fromName);
         DPCOverlayAddText(buf, SHOW_HILIGHT_TIME);
      }
   }
...
void DPCBroadcastHilightObject(ObjID obj)
{
...
//
static void DPCPlayerCrash(DWORD data)
{
#define MAX_TEMPLATE_LEN 64
   ...
   if (player == OBJ_NULL)
   {
      ...
      if (DPCStringFetch(templ, MAX_TEMPLATE_LEN,
                           "UnknownDisconnected", "Network", -1))
      {
         DPCSendAddText(OBJ_NULL, templ, 5000);
         DPCOverlayAddText(templ, 5000);
      }
                        ...
                        "PlayerDisconnected", "Network", -1))
   {
      ...
      DPCSendAddText(OBJ_NULL, msgBuf, 5000);
      DPCOverlayAddText(msgBuf, 5000);
   }
        ...
        pContainSys->IterNext(pIter))
   {
      ...
      if ((currentBox == OBJ_NULL) ||
          (!DPCInvFindSpace(containerContents, boxDims, obj, &slot)))
      {
         ...
         if (!DPCInvFindSpace(containerContents, boxDims, obj, &slot)) {
      ...
      DPCInvFillObjPos(obj, containerContents, slot, boxDims);
   }
...
static void DPCNetAddText(const char *msgName, int time)
{
   char msg[MAX_NETSTRING_LEN];
   if (DPCStringFetch(msg, MAX_NETSTRING_LEN, msgName, "Network", -1)) {
...
static void DPCNetworkLost()
{
   if (IsSimTimePassing()) {
      DPCNetAddText("NetworkLost", 30000);
   }
}
...
//
static void DPCPlayerIconSetup()
{
...
static void DPCNetDisplayPause(BOOL paused,
                                 BOOL enterGame,
                                 void * /* pClientData */)
{
   if (paused) {
      if (enterGame) {
         DPCNetAddText("NetWait", 1);
         g_enteringGame = TRUE;
      } else {
         DPCNetAddText("NetPause", 1);
         g_enteringGame = FALSE;
      }
   } else {
      if (g_enteringGame) {
         DPCNetAddText("NetJoined", 5000);
      } else {
         DPCNetAddText("NetResume", 5000);
      }
   }
}
...
// be consistent.
//
int DPCPlayerNumColor(int playerNum)
{
   ...
   switch(playerNum)
   {
      ...
      default:
         return gDPCTextColor;
   }
}

int DPCPlayerColor(ObjID playerObj)
{
   AutoAppIPtr(NetManager);
   return DPCPlayerNumColor(pNetManager->ObjToPlayerNum(playerObj));
}
...
void DPCNetInit()
{
   ...
   IAIBehaviorSet *pBehaviorSet = new cAIDPCProxyBehaviorSet;
   ...
   NetSimRegisterCallback(DPCNetDisplayPause, NULL);
...
// Shut down Deep Cover-specific networking. Doesn't currently need to do
// anything.
void DPCNetTerm()
{
