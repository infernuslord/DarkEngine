...
const float kDPCLandForce = 3600;
const float kDPCFallDmgRatio = 1800;
...
eDamageResult LGAPI DPCDamageListener(const sDamageMsg* pMsg, tDamageCallbackData data)
{
   switch(pMsg->kind)
   {
   ...
   case kDamageMsgImpact:
      {
         ...
         if (textureID>=0)
         {
            ...
            if (pMsg->victim != PlayerObject())
            {
               ...
               IRelation *pRel = pLinkManager->GetRelationNamed("Corpse");
               if (pTrait == NULL)
               {
                   pTrait = MakeTraitFromRelation(pRel);
               }
               ILinkQuery *query = QueryInheritedLinksSingle(pTrait,pRel,texarch, LINKOBJ_WILDCARD); 
               while (!query->Done())
               {
                  query->Link(&slink);
         ...
         // we can move it into a DPCProj... fn if we need to
         // This just 
   ...
   case kDamageMsgSlay:
      {
         ...
         if (exp != 0)
         {
            ...
            pDPCPlayer->AddExperience(PlayerObject(), exp);
         }
         */
